...
function MakeGuiConfig()
	...
	local itemLists = {}
	local namesSeen = {}
	for iname, objid in pairs(Gatherer.Nodes.Names) do
		local name = Gatherer.Util.GetNodeName(objid)
		local objType = Gatherer.Nodes.Objects[objid]
		--if (not name or not objType) then
			--print( "missing data for ", iname, objid, name, objType )
		--end
		local gtype = objType:lower()
