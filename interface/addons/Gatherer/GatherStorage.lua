--[[
	...
	Revision: $Id: GatherStorage.lua 1148 2016-08-18 22:48:50Z ccox $
...
--]]
Gatherer_RegisterRevision("$URL: http://svn.norganna.org/gatherer/tags/REL_7.3.1/Gatherer/GatherStorage.lua $", "$Rev: 1148 $")
...
function lib.AddNode(nodeName, gatherType, inMapID, gatherX, gatherY, source, incrementCount, indoorFloor)
	...
	local zoneToken = Gatherer.ZoneTokens.GetZoneToken(inMapID)
	...
	for i, nodeData in ipairs(gtypeTable) do
		if ( nodeData[FLOOR] == indoorFloor ) then
			...
			if ( dist and dist < matchDist ) then
				...
				for index, nodeData in ipairs(nodesList) do
					...
					if ( returnNode ) then
						...
						if ( (nodeX ~= 0) and (nodeY ~= 0) ) then
							...
							if ( dist and ((maxDist == 0) or (dist < maxDist)) ) then
								local insertPoint = 1
