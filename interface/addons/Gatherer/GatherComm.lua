--[[
	...
	Revision: $Id: GatherComm.lua 1148 2016-08-18 22:48:50Z ccox $
...
Gatherer_RegisterRevision("$URL: http://svn.norganna.org/gatherer/tags/REL_7.3.1/Gatherer/GatherComm.lua $", "$Rev: 1148 $")
...
function Gatherer.Comm.Send( objectId, gatherType, indoorNode, gatherZ, gatherX, gatherY, nodeIndex )
	if ( type(objectId) == "number" ) then
		...
		if not (lastHarvest and lastHarvest.z == zoneToken and lastHarvest.o == objectId and lastHarvest.i == nodeIndex) then
			...
			local sendMessage = strjoin(";", objectId, tostring(zoneToken), tostring(indoorNode and true or false), gatherX, gatherY)
