--[[
	...
	Revision: $Id: GatherEvent.lua 1154 2016-09-09 00:14:54Z ccox $
...
--[[  Old code, no longer reliable because of string use changes
		...
		--if (skill) then print("skill matched err_use_locked_with_item_s") end
		if not ( skill ) then
			skill = Gatherer.Util.ParseFormattedMessage(ERR_USE_LOCKED_WITH_SPELL_KNOWN_SI, msg)
			--if (skill) then print("skill matched err_use_locked_with_spell_known_si") end
		end
		if not ( skill ) then
			skill = Gatherer.Util.ParseFormattedMessage(LOCKED_WITH_SPELL_KNOWN, msg)	-- Legion?
...
-- ]] --
		...
		if (err_number == LE_GAME_ERR_USE_LOCKED_WITH_SPELL_S) then
			...
			--local errorName, soundKitID, voiceID = GetGameMessageInfo(err_number)
			--DebugPrintQuick("Gatherer REQUIRE event: ", tip, objId, err_number, msg, tooltip, errorName, ... )
			if ( objId ) then
...
function Gatherer.Event.OnSwag(lootType, lootTable, coinAmount, extraData)
	...
	if (lootType and lootType ~= "KILL") then
		...
		--DebugPrintQuick("Gatherer SWAG event: ", lootType, lootTable, coinAmount, extraData, node )
		...
		--DebugPrintQuick("Gatherer calling AddGather: ", object, lootType )
